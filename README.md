...
Verify the account using the verification link/code.
Login with email and password.
Use the returned access token for protected requests.
Use the refresh token cookie to refresh expired access tokens.
Logout when finished.
Example Agent Flow
Create an agent.
Upload a PDF file to the agent.
Trigger agent training.
Wait until the agent AI status becomes ready.
Test the agent using a message.
Create a widget for public usage.
Security Notes
